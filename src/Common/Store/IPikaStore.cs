...
namespace Pika.Common.Store
{
    public interface IPikaStore
    {
        ...
        Task<List<PikaScriptRun>> GetScriptRunsByScriptIdAsync(long scriptId, int limit = int.MaxValue);
        Task<List<PikaScriptRun>> GetScriptRunsByStatusAsync(PikaScriptStatus status, bool desc = true);
        Task<List<PikaScript>> GetScriptsAsync();
